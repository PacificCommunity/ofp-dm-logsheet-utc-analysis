/**
 * Data loader: ll-observer-offset-per-eez.csv.js
 *
 * For each EEZ code present on Longline fishing sets, produces:
 *   - Summary counts: total_sets, total_trips, observer_trips (per EEZ)
 *   - Observer UTC offset distribution: observer_offset + count
 *     (derived from obsv.l_set.set_dtime − utc_set_dtime, matched by obstrip_id + date)
 *
...
-- ── Per matched set: observer UTC offset ─────────────────────────────────────
observer_offsets AS (
    SELECT
        sl.eez_code,
        ROUND(
