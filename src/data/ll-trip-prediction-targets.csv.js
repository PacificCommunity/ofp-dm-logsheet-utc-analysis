/**
 * Data loader: ll-trip-prediction-targets.csv.js
 *
 * Longline logsheet trips that have NO linked observer trip — these are the
...
-- ── All LL trips that DO have an observer link ────────────────────────────────
ll_observer_linked AS (
    SELECT DISTINCT dto_guid_left  AS log_trip_id FROM tufman2.entity_links
    WHERE dto_type_left = 'LonglineLogsheetDTO' AND dto_type_right = 'ObserverTripDTO'
    UNION
    SELECT DISTINCT dto_guid_right AS log_trip_id FROM tufman2.entity_links
    WHERE dto_type_right = 'LonglineLogsheetDTO' AND dto_type_left = 'ObserverTripDTO'
),
...
ll_eez_counts AS (
    ...
    WHERE sl.l_activity_id = 1
      ...
      AND NOT EXISTS (SELECT 1 FROM ll_observer_linked ol WHERE ol.log_trip_id = tl.log_trip_id)
...
-- ── All unresolved LL trips with features ─────────────────────────────────────
SELECT
    tl.log_trip_id,
